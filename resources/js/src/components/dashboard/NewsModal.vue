    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        >
          ...
          <span v-else>{{ isEdit ? 'Simpan Perubahan' : 'Tambah Berita' }}</span>
        </button>
      </div>
    </template>
  </BaseModal>
...
// ─── Sync form saat news prop berubah ───────────────────────────
watch(() => props.news, (newNews) => {
  if (newNews) {
    form.value = {
      title: newNews.title,
      description: newNews.description,
      image: newNews.image, // ← langsung pakai string, tidak dikosongkan
    }
    previewImage.value = newNews.image
  } else {
    resetForm()
  }
}, { immediate: true })
...
const validateForm = (): boolean => {
  errors.value = {}
  if (!form.value.title.trim()) errors.value.title = 'Judul berita wajib diisi'
